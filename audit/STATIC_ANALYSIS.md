...
| X-011 | `unwrap` / `panic!` / `unimplemented!` in production code | **Pass** | None outside `#[cfg(test)]` modules or Rust integration-test files (`programs/*/tests/*.rs`, e.g. `programs/zettapay-core/tests/integration.rs`). The scanner's awk filter strips `#[cfg(test)] mod` blocks and the file-listing step excludes the per-crate `tests/` directory entirely (whole-file test crates). | Every new handler must use `?` error propagation; a single production `unwrap` is a DoS surface. |
